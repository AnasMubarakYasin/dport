...
import { Promiseify, toQueryString } from './helper';

export type Version = "v0" | "lts" | "latest";
export type Message<M = any> = {
	tag: string;
	data: M;
};
...
export type { ClientWebSocket, ClientServerSentEvent };
...
class ClientServerSentEvent {
	...
	protected internal_onmessage = (event: MessageEvent) => {
		const data: Message = this.derializer(event.data);
