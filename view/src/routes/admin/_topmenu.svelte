		...
		<SList class="p-2 gap-2">
			{#each notification as item}
				<SListItem class="w-full">
					<button class="w-full px-3 py-2 hover:bg-base-200 active:bg-primary rounded-md text-left"
						>{item.name}</button
					>
				</SListItem>
			{/each}
	...
	<Popup for="account" auto_click right={false} class="w-80 bg-base-100">
		...
		<SList class="p-2 gap-2">
			{#each navigation as item}
				<SListItem class="w-full">
					<a
						class="block link link-hover px-3 py-2 hover:bg-base-200 active:bg-primary rounded-md"
						href={item.link}
					>
						{item.name}
					</a>
